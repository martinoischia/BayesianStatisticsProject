...
rates = list(sqrt(1000), sqrt(40), sqrt(10), sqrt(2.5), sqrt(0.625))

for (i in rates){
  set.seed(42)
  data_toy <- c(rgamma(n, alpha, rate=i))
  ...
  data.frame(x = seq(min(model$grideval), max(model$grideval), by = 0.01), 
             y = dgamma(seq(min(model$grideval), max(model$grideval), by = 0.01), alpha, rate=i)) %>%
    ggplot()+
    theme_minimal() + 
