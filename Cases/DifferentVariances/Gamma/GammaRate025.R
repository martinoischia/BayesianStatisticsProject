...
##### SIMULATION OF DATA TOY OF 1 GAMMA DIFFUSE DISTRIBUTION.

set.seed(42)

# Variance is given by alpha/(beta^{2})
data_toy <- c(rgamma(15, 10, rate=0.25))
...
data.frame(x = seq(min(model$grideval), max(model$grideval), by = 0.01), 
           y = dgamma(seq(min(model$grideval), max(model$grideval), by = 0.01), 10, rate=0.25)) %>%
  ggplot()+
  theme_minimal() + 
